...
for i in ${V_ticket[@]}
do
ansible-playbook display_data. --extra-vars="myvarfile=${i}.yaml"
done
...
Destination_cluster_avaiable_memory=`cat cluster_info.json | grep -w "Available Memroy (GB)" | awk -F ": " '{print $2}'`
echo "Destination_cluster_avaiable_memory(GB): $Destination_cluster_avaiable_memory"
echo""
echo""
Destination_cluster_avaiable_cpu=`cat cluster_info.json | grep -w "Available CPU (Mhz)" | awk -F ": " '{print $2}'`
echo "Destination_cluster_avaiable_cpu(Mhz): $Destination_cluster_avaiable_cpu"
echo""
echo""
Memory_requirement_for_VM_migration=`cat vminfo.json | grep MemAllocated | awk -F ": " '{print $2}'`
echo "Memory_requirement_for_VM_migration(GB): $Memory_requirement_for_VM_migration"
echo""
echo""
CPU_requirement_for_VM_migration=`cat vminfo.json | grep "CPU(Mhz)" | awk -F ": " '{print $2}'`
echo "Average CPU utilization of VM(Mhz): $CPU_requirement_for_VM_migration"
echo""
echo""
